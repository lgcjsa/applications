...
import luiss.it.mail.MailWithAttachment;
import luiss.it.mail.db.ManageTesti;

@SuppressWarnings("serial")
public class Internazionali_L2 extends Index {
	...
	public Internazionali_L2() {
		...
		this.diplomi.add("General Baccalaurerat");
	...
	public void conferma() throws Exception {
		...
		if (new ManagePreweb().getByID(keyPreweb) == null) {
			...
			Mail mail = new MailWithAttachment();
			mail.setArg(preweb);
			mail.setOggetto("LUISS Guido Carli Application");
			mail.setTesto(new StringBuffer(new ManageTesti().getByID(4).getTesto()));
