 ...
 * @see luiss.it.LGCDB_Clienti.dao.Log
...
public class LogHome {
	...
	public Log merge(Log detachedInstance) {
		...
		try {
			Log result = (Log) sessionFactory.getCurrentSession().merge(detachedInstance);
	...
	public Log findById(java.lang.Integer id) {
		...
		try {
			Log instance = (Log) sessionFactory.getCurrentSession().get("luiss.it.LGCDB_Clienti.dao.Log", id);
			if (instance == null) {
	...
	public List findByExample(Log instance) {
		...
		try {
			List results = sessionFactory.getCurrentSession().createCriteria("luiss.it.LGCDB_Clienti.dao.Log")
