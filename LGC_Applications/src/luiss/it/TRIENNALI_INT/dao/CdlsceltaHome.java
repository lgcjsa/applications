 ...
 * @see luiss.it.TRIENNALI_INT.dao.Cdlscelta
...
public class CdlsceltaHome {
	...
	public Cdlscelta findById(luiss.it.TRIENNALI_INT.dao.CdlsceltaId id) {
		log.debug("getting Cdlscelta instance with id: " + id);
		try {
			Cdlscelta instance = (Cdlscelta) sessionFactory.getCurrentSession()
					.get("luiss.it.TRIENNALI_INT.dao.Cdlscelta", id);
			if (instance == null) {
	...
	public List findByExample(Cdlscelta instance) {
		...
		try {
			List results = sessionFactory.getCurrentSession().createCriteria("luiss.it.TRIENNALI_INT.dao.Cdlscelta")
