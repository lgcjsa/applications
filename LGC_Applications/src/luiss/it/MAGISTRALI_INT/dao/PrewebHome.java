 ...
 * @see luiss.it.MAGISTRALI_INT.dao.Preweb
...
public class PrewebHome {
	...
	public Preweb findById(luiss.it.MAGISTRALI_INT.dao.PrewebId id) {
		log.debug("getting Preweb instance with id: " + id);
		try {
			Preweb instance = (Preweb) sessionFactory.getCurrentSession().get("luiss.it.MAGISTRALI_INT.dao.Preweb", id);
			if (instance == null) {
	...
	public List findByExample(Preweb instance) {
		...
		try {
			List results = sessionFactory.getCurrentSession().createCriteria("luiss.it.MAGISTRALI_INT.dao.Preweb")
